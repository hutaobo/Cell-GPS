...
def style_axes(ax):
    ...
    ax.tick_params(colors=PALETTE["ink"], labelsize=6)
...
def figure_combined_lead_in(metadata, counts, summary, overall, cluster_counts):
    ...
    # Nature double-column width is 180 mm (7.087 in); height stays within the
    # 247 mm single-page limit. Exact canvas + no tight bbox keeps fonts at true pt.
    fig = plt.figure(figsize=(7.087, 7.18), dpi=600)
    gs = GridSpec(
        4,
        6,
        figure=fig,
    ...
    ax.set_ylabel("transcripts (millions)")
    ax.set_title("Molecular scale", fontsize=7.0, pad=4)
    ax.text(
        0.98,
        0.78,
        "19.7%\nhigh-quality\nuRNAs",
    ...
    ax.set_title("Analysis universe", fontsize=7.0, pad=4)
    ...
    ax.set_xlabel("uRNA fraction per gene")
    ax.set_ylabel("genes")
    ax.set_title("Full-panel uRNA coverage", fontsize=7.0, pad=4)
    ...
    ax.set_title("uRNA-only COSTE best cell-group calls", fontsize=7.0, pad=4)
    style_axes(ax)
    ...
    ax.set_title("Global concordance, summarized", fontsize=7.0, pad=4)
    ...
    ax.set_title("Marker-control logic", fontsize=7.0, pad=4)
    panel_label(ax, "f", x=-0.020, y=1.08)
    ...
    for idx, (gene, target, status) in enumerate(example_cards):
        ...
        ax.text(x0 + width / 2, 0.76, gene, ha="center", va="center", fontsize=7.0, fontweight="bold", color=color)
        ...
        x0 += width + 0.018
    ax.set_title("Bridge to spatial evidence", fontsize=7.0, pad=4)
    panel_label(ax, "g", x=-0.020, y=1.08)

    fig.subplots_adjust(left=0.078, right=0.985, top=0.957, bottom=0.050)
    # No bbox_inches="tight": keep the canvas exactly 180 mm wide so text stays at
    # true point size and the figure does not need rescaling at the journal's width.
    fig.savefig(FIGURES / "figure_1_integrated_urna_evidence_chain.png")
    fig.savefig(FIGURES / "figure_1_integrated_urna_evidence_chain.pdf")
    plt.close(fig)


def figure_supplementary_discordance(summary):
    mm = 1 / 25.4
    fig, axes = plt.subplots(1, 2, figsize=(183 * mm, 88 * mm), dpi=600)
    fig.suptitle(
        "Supplementary Figure 1. Discordant uRNA-COSTE profiles are not explained only by low coverage",
        x=0.01,
        y=0.98,
        ha="left",
        va="top",
    ...
    fig.savefig(FIGURES / "supplementary_figure_1_discordant_genes.png", bbox_inches="tight")
    fig.savefig(FIGURES / "supplementary_figure_1_discordant_genes.pdf", bbox_inches="tight")
    plt.close(fig)
