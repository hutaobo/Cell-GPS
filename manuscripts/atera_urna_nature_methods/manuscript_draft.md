...
a, Transcript counts through gene, quality and uRNA filtering. b, Analysis-set definition from the 18,028-gene panel to the 530 genes analyzed by uRNA-only COSTE and concordance; only these 530 genes enter COSTE/concordance. c, Distribution of gene-level uRNA fractions across all panel genes, with the 530 analyzed genes overlaid. d, uRNA-only best COSTE cell groups for the 530 analyzed genes. e, Compact summary of global concordance between uRNA-only and all-transcript SSS profiles across the 530 analyzed genes; middle-ground agreement indicates that uRNAs recover many compartments without copying the cell-assigned signal. f, Curated marker-control logic showing expected compartments recovered from uRNAs alone and the heterogeneous tumor/DCIS interface control; curated markers define interpretable positive controls before local spatial inspection. g, Spatial-evidence bridge showing the four concordant positive examples and the discordant ERBB2 control carried into Figure 2, where the five choices are visualized with true uRNA and cell coordinates. Detailed five-gene SSS heatmaps, concordance placement and per-example statistics are shown only in Figure 2.
