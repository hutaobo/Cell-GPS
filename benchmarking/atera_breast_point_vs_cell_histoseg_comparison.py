...
def classify_pathology(celltype: str = "", genes: str = "") -> str:
    ...
    if immune and stroma:
        return "immune/stroma interface"
    if tumor and immune:
...
def plot_heatmap(row_fraction: pd.DataFrame, output_dir: Path) -> None:
    ...
    for i in range(row_fraction.shape[0]):
        for j in range(row_fraction.shape[1]):
            ...
            if value >= 0.05:
                ax.text(j, i, f"{value:.2f}", ha="center", va="center", color="white" if value > 0.25 else "black", fontsize=7)
...
def plot_spatial_comparison(inputs: dict[str, Any], preview: pd.DataFrame, output_dir: Path) -> None:
    ...
    for ax, x, y, color, title, size, alpha in panels:
        scatter = ax.scatter(x, y, c=color, s=size, cmap="tab20", linewidths=0, alpha=alpha, rasterized=True)
        ax.set_aspect("equal")
        ax.invert_yaxis()
        ax.set_xlabel("x (um)")
        ax.set_ylabel("y (um)")
...
def write_report(outputs: dict[str, Any], distance_stats: dict[str, Any], output_dir: Path) -> None:
    ...
    lines.extend(
        [
            ...
            "",
            "The largest point domains are dominated by COSTE signatures such as SERPINA6/MSMB/KCNQ3/SERPINA1 and RERGL/BMPER/IGF2/DPT/CCL22. These signatures map mainly to tumor-rich, DCIS/luminal, stromal/CAF, immune-rich, and immune/stroma-interface cell-level regions depending on their nearest cell-domain context. This supports the view that the point-level HistoSeg is detecting molecular micro-regions rather than simply reproducing the cell-level segmentation.",
            "",
        ]
    )
